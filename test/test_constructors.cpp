...
TEST_CASE("Fill construct a small vector of floats" * test_suite("constructors")) {
  ...
  REQUIRE(vec.stack_size() == 10);
  REQUIRE(vec.heap_size() == 10);
  std::size_t count = 0;
...
TEST_CASE("Copy construct a small vector of strings" * test_suite("constructors")) {
  ...
  REQUIRE(bar.size() == 6);
  REQUIRE(bar.stack_size() == 5);
  REQUIRE(bar.heap_size() ==1);
  REQUIRE(bar.capacity() == 6);
  REQUIRE(bar[0] == "a");
  REQUIRE(bar[1] == "b");
  REQUIRE(bar[2] == "c");
  REQUIRE(bar[3] == "d");
...
TEST_CASE("Move construct a small vector of chars" * test_suite("constructors")) {
  ...
  REQUIRE(bar.size() == 3);
  REQUIRE(bar.stack_size() == 5);
  REQUIRE(bar.heap_size() ==0);
  REQUIRE(bar.capacity() == 5);
  REQUIRE(bar[0] == 'a');
  REQUIRE(bar[1] == 'b');
  REQUIRE(bar[2] == 'c');
}
...
TEST_CASE("Construct a small vector of bools from initializer list" * test_suite("constructors")) {
  ...
  REQUIRE(foo.size() == 8);
  REQUIRE(foo.stack_size() == 5);
  REQUIRE(foo.heap_size() == 3);
  REQUIRE(foo.capacity() == 8);
  REQUIRE(foo[0] == true);
  REQUIRE(foo[1] == false);
  REQUIRE(foo[2] == true);
  REQUIRE(foo[3] == false);
